...
//      cylinder test case
//           rpart[jad] = sqrt((rpart[jx0]-5)**2+(rpart[jy0]-5)**2)
        int factor = 0;
        if (j==0){
           factor = 1;
           rpart[jx0] = rpart[jx0] + (1.0/3)*rpart[jaa]*dt*factor*(sqrt(pow(rpart[jx0]-5,2)+pow(rpart[jx0+1]-5,2))); //*rpart[jad]
        }
        else if(j == 1){
           factor = 1;
           rpart[jx0] = rpart[jx0] + (1.0/3)*rpart[jaa]*dt*factor*(sqrt(pow(rpart[jx0-1]-5,2)+pow(rpart[jx0]-5,2))); //*rpart[jad]
       }
       else{
           factor = 0;
           rpart[jx0] = rpart[jx0];
