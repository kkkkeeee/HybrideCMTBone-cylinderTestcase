...
c       cenPos(1) = rand(70886)*(xdrange(2,1)-xdrange(1,1))
c       cenPos(2) = rand(0)*(xdrange(2,2)-xdrange(1,2))
c       cenPos(3) = rand(0)*(xdrange(2,3)-xdrange(1,3))
...
cc          new curtain test case
c           rpart(jx, i) = rand(0)*10.0
c           rpart(jy, i) = rand(0)*0.1
c           rpart(jz, i) = rand(0)
...
c          cylinder test case
           tempa = (rand()-0.5)*2  !generate from (-1,1)
           tempb = (rand()-0.5)*2  !generate from (-1,1)
           tempd = sqrt(tempa**2+tempb**2)
           rpart(jaa, i) = tempa/tempd
           rpart(jab, i) = tempb/tempd

           tempc = rand()
           rpart(jx, i) = 5 + 1.0/2*rpart(jaa, i)*tempc !a circle of 1/2 radius
           rpart(jy, i) = 5 + 1.0/2*rpart(jab, i)*tempc !with center(5,5)
           rpart(jz, i) = rand()
