...
c         rpart(jy, i) = rpart(jy, i)
c         rpart(jz, i) = rpart(jz, i)

cc         new curtain test case
c          rpart(jx, i) = rpart(jx, i) 
c          rpart(jy, i) = rpart(jy, i) + (1.0/3)*rpart(jy, i)*DT
c          rpart(jz, i) = rpart(jz, i)

c         cylinder test case
          rpart(jad, i) = sqrt((rpart(jx, i)-5)**2+(rpart(jy, i)-5)**2) !r
c         print *, 'nid: ', nid, 'i:', i, lglel(ipart(je0,i)+1),
c    $        rpart(jx, i), rpart(jy,i), rpart(jz,i)
c    $        ,rpart(jx, i) + (1.0/3)*rpart(jaa, i)*rpart(jad,i)*dt,
c    $        rpart(jy, i) + (1.0/3)*rpart(jab, i)*rpart(jad, i)*dt

          rpart(jx, i) = rpart(jx, i) +
     $               (1.0/3)*rpart(jaa, i)*rpart(jad,i)*dt
          rpart(jy, i) = rpart(jy, i) +
     $               (1.0/3)*rpart(jab, i)*rpart(jad, i)*dt
